...
<div class="title-bar">
    ...
    <div class="time-badge">
        <div class="time-main">{now.strftime('%I:%M %p')}</div>
        <div class="time-sub">{now.strftime('%a, %d %b %Y')}</div>
    </div>
</div>
""", unsafe_allow_html=True)
...
with tab_map:
    ...
    if load_map or st.session_state.get("load_traffic_map"):
        ...
        if selected_html:
            ...
            fall_html = _read_html(FALLBACK_MAP)
            if fall_html:
                st.markdown("**Traffic Map - Kothrud, Pune**")
                components.html(fall_html, height=540, scrolling=False)
            else:
                st.markdown("""
                <div class="api-warning">
